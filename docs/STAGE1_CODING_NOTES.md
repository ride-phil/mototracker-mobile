...
`peerDependencies`, requires a new EAS build. Always flag this before suggesting.

**Packages added in Stage 2 Session 1 (new dev client build triggered):**
- `expo-notifications`
- `react-native-gesture-handler`
- `react-native-reanimated`

> The custom `AppDrawer` was originally built to avoid the gesture-handler/reanimated
> dependency. Both are now in the build but the custom drawer is still in use.
